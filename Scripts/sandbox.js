...
function createProjectSection(project) {
  ...
  // Create media content (images and videos)
  const mediaContent = [...(project.images || []), ...(project.videos || [])]
    .map((media) => {
      if (media.type === "video") {
        return `
          <video 
        ...
        return `<img src="${media.url}" alt="${media.alt}" class="gallery-img">`;
      }
    })
    .join("");
  ...
  // Add click handlers for images and videos after the HTML is set
  const images = section.querySelectorAll(".gallery-img");
